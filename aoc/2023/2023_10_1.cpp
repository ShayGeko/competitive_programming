...
void p2() {
	...
	ans = 0;
	vvi vert(sz(grid)+1, vi(sz(grid[0])+1, 0));
	auto hor = vert;
	rep(i, 1, sz(vert)) {
		rep(j, 1, sz(vert[0])) {
			// cout << isLoop[i-1][j-1];
			...
			hor[i][j] = hor[i][j-1];
			if(isLoop[i-1][j-1]) {
				if(grid[i-1][j-1] != '|') vert[i][j]++;
				if(grid[i-1][j-1] != '-') hor[i][j]++;
			}
		}
	}
	int n = sz(vert), m = sz(vert[0]);
	ans = 0;
	rep(i, 1, n) {
		rep(j, 1, m) {
			if(!isLoop[i-1][j-1]) {
				auto vpref = vert[i][j];
				auto vsuf = (2 + vert[n - 1][j] - vert[i][j]) ;
				auto hpref = hor[i][j];
				auto hsuf = (2 + hor[i][m - 1] - hor[i][j]);

				if(vpref > 0 && vsuf > 0 && hpref > 0 && hsuf > 0 && vpref % 2 == 1 && vsuf % 2 == 1 && hpref % 2 == 1 && hsuf % 2 == 1) {
					++ans;
					cout << '1';
				} else cout << 0;
			} else cout << '#';
