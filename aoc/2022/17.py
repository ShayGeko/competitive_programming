...
def signature(R):
    maxY = max([y for (x,y) in R])
    return frozenset([(x,maxY-y) for (x,y) in R if maxY-y<=200])
...
while t<L:
    ...
    while True:
        ...
        # if something in intersection -> collision
        if piece & R:
            piece = move_up(piece)
            R |= piece
            ...
            if SR in SEEN and t>=2022:
                (oldt, oldy) = SEEN[SR]
                dy = top-oldy # pattern size
                dt = t-oldt   # number of rocks in between
                amt = (L-t)//dt # how many times can this pattern fit in
                added += amt*dy # add as many such patterns as we can to answer
                t += amt*dt     # add number of rocks per pattern times num_patterns
                assert t<=L     # if t greater than L gotta break?
            SEEN[SR] = (t,top)
            #show(R)
            break
    t += 1
