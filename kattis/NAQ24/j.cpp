...
int main() {
	...
	rep(l, 0, L) {
		...
		rep(i, 0, 3) {
			// handle boundaries with center losing on equality
			if(coord_frac[i] < boundaries[0] || coord_frac[i] == boundaries[0]) cubecoord[i]=0;
			else if(coord_frac[i] < boundaries[1])cubecoord[i]=1;
			else cubecoord[i] = 2;
		}
